  ...
  return (
    <div className="w-[150px] ml-4 bg-gray-400 cursor-pointer text-center drop-shadow-md shadow-black text-white rounded text-sm">
      <form >
        ...
        <input className= "bg-gray-400" type="text" name="from" id="from" onChange={e => onChangeHandler(e.target.value)} value={text} />
        {suggestions && suggestions.map((suggestion, i) =>
        <div key={i} onClick= {()=> OnSuggestHandler(suggestion)}>  {suggestion}</div>
        )}        
      </form>
    </div>
  );
